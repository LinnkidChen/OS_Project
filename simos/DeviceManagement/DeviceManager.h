...
public:
    ...
    // 0，磁盘繁忙
    // 1，存储成功
    static std::string disk_read(int address,
                                 int size_byte); //磁盘读（地址，内容）
    static std::string disk_read(int deviceID, int address,
...
public:
    ...
    static char keyboard_read() { return Device_Keyboard::keyboard_read(); }
