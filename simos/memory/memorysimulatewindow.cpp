...
MemorySimulateWindow::MemorySimulateWindow(QWidget *parent)
    : QWidget(parent), ui(std::make_unique<Ui::MemorySimulateWindow>()) {
    ui->setupUi(this);
    this->hide();
    ...
    connect(ui->pushButton, &QPushButton::clicked, [=]() {
        ...
        int num = rand() % 4 + 6;
        srand(
            (int)time(0)); //利用系统时钟，产生不同的随机数种子,把0换成NULL也行
        for (int i = 1; i <= num; i++) {
