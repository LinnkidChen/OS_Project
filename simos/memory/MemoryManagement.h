...
// 页的数量
    ...
    static constexpr const size_t SWAP_FILE_SIZE = 4096;
    static constexpr const size_t VIRTUAL_PAGE_NUM = 4;
...
/// 使用MemoryPool时，调用MemoryPool::GetInstance()方法来获取实例。
    ...
    private:
        ...
        size_t replacePage(size_t needSize) noexcept;
    ...
    private:
        ...
        std::unordered_map<long long, long long> page_table;

        std::array<page_type, MEMORY_PAGE_NUM>   virtual_pages;
        ConstQueue<page_type *, MEMORY_PAGE_NUM> free_virtual_pages;

        std::array<page_type, SWAP_FILE_SIZE>   swap_file;
        ConstQueue<page_type *, SWAP_FILE_SIZE> free_swap_file;

        struct Node{
            int page_num;
            Node *left,*right;
            Node(int _page_num): page_num(_page_num),left(NULL),right(NULL){}
        };
