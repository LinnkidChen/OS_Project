...
std::vector<int> MainWindow::create_rand_pagenums_vector()//取得[a,b)的随机整数
{
    int a =0,b = 4096;
    std::vector<int> res;
    srand((int)time(0));
...
void MainWindow::removeNode(Node* p)
{
    p->left->right = p->right;
    p->right->left = p->left;
}

void MainWindow::insertNode(Node* p)
{
    p->right = qhead->right;
    p->left = qhead;
    qhead->right->left = p;
...
void MainWindow::LRUCache(int capacity)
{
    n = capacity;
    qhead = new Node(-1);
    qtail = new Node(-1);
